...
class NCC():
    ...
    def ncc(self, I, J):
        ...
        if self.win is None:
            ...
            USEFULL CODE : https://colab.research.google.com/drive/1ePIEXzGWL0sp2xsW3JhVYuL2lXFEODQ7#scrollTo=SAsmE1TSi5HM
        ...
        sum_filt = torch.ones((batch_size, channels, *self.win)).float()
        strides = [1] * (ndims)
        # compute local sums via convolution
...
class Grad():
    ...
    def _diffs(self, y):
        ...
        for i in range(ndims):
            ...
            # PyTorch
            df[i] = torch.Tensor(dfi).permute(r)
...
class SparseVM(object):
    ...
    def sparse_conv_cc3D(self, atlas_mask, conv_size=13, sum_filter=1, padding='same', activation='elu'):
        ...
        def loss(I, J):
            ...
            not_used, not_used_mask, I2_sum, conv_mask = self.conv_block(I2, combined_mask, im_conv, mask_conv,
                                                                         'I2_sum')
            not_used, not_used_mask, J2_sum, conv_mask = self.conv_block(J2, combined_mask, im_conv, mask_conv,
                                                                         'J2_sum')
            not_used, not_used_mask, IJ_sum, conv_mask = self.conv_block(IJ, combined_mask, im_conv, mask_conv,
                                                                         'IJ_sum')
